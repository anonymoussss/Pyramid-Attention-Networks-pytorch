...
from networks import PAN, ResNet50, Mask_Classifier
...
def train(epoch, optimizer, data_loader):
    ...
    for batch_idx, (imgs, mask_labels, img_name) in enumerate(data_loader):
        ...
        fms_blob, z = convnet(imgs)  #fms_blob:list
        #out_cls = classifier(z.detach())
        
        ##############   debug   ###################
        #print(len(fms_blob))    #4
        #print(fms_blob[0].cpu().detach().numpy().shape)
        #print(fms_blob[1].cpu().detach().numpy().shape)
        #print(fms_blob[2].cpu().detach().numpy().shape)
        #print(fms_blob[3].cpu().detach().numpy().shape)
        #################################
        ...
        out_ss = pan(fms_blob[::-1])
        ##############   debug   ###################
        #out_ss_npy = out_ss.cpu().detach().numpy()
        #nonzero_outss_indice = np.where(out_ss_npy>0)
        #print(out_ss_npy[nonzero_outss_indice])
        #print(out_ss_npy.shape)  #(4,256,64,64)
        #################################
        ...
        ##############   debug   ###################
        #mask_pred_npy = mask_pred.cpu().detach().numpy()
        #nonzero_pred_indice = np.where(mask_pred_npy>0)
        #print(mask_pred_npy[nonzero_pred_indice])
        #print(mask_pred_npy.shape)  #(4,21,64,64)
        #################################
        
        mask_labels = F.interpolate(mask_labels, scale_factor=0.25, mode='nearest')
        ##############   debug   #################
        #mask_labels_npy = mask_labels.cpu().numpy()
        #nonzero_labels_indice = np.where(mask_labels_npy>0)
        #print(mask_labels_npy[nonzero_labels_indice])
        #print(mask_labels_npy.shape) #(4,1,64,64)
        #################################
        ...
        # Result
        #a = mask_pred.max(dim=1)[1].data.cpu() ->dtype:torch.int64
        #b = mask_labels.long().squeeze(1).cpu() ->dtype:torch.float32
        pixel_acc += mask_pred.max(dim=1)[1].data.cpu().eq(mask_labels.long().squeeze(1).cpu()).float().mean()
        ...
        if (batch_idx+1) % 64 == 0:
            ...
            logging.info(
                ...
                "Train Epoch:{:}, {:}/{:}, loss_ss:{:.4f}, pixel_acc:{:.4f}%".format(
                    epoch, args.batch_size*batch_idx, length_training_dataset, loss_ss.data, pixel_acc/batch_idx*100))
def test(data_loader):
    ...
    logging.info("Length of test set:{:} Each_cls_IOU:{:} mIOU:{:.4f} PA:{:.4f}".format(length_test_dataset, dict(zip(test_data.classes, (100*each_cls_IOU).tolist())), mIOU*100, pixel_acc))
